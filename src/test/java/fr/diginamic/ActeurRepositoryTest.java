...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r WHERE r.film.annee = 2015", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film.pays p WHERE p.nom = 'France'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film.pays p WHERE p.nom = 'France' AND r.film.annee = 2017", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film.realisateurs real WHERE real.identite = 'Ridley Scott' AND r.film.annee BETWEEN 2010 AND 2020", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireRealisateursParActeur() {
		TypedQuery<Realisateur> query = em.createQuery("SELECT DISTINCT r FROM Realisateur r JOIN r.films f JOIN f.roles ro JOIN ro.acteur a WHERE a.identite = 'Brad Pitt'", Realisateur.class);
